...
const T20Content = () => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const allMetBets = response.data.map(bet => ({
        ...
        stake: bet.stake,
        noRuns: bet.mode === "no" ? bet.noRuns : 0,
        yesRuns: bet.mode === "yes" ? bet.yesRuns : 0,
        result: bet.result,
        winningRuns: bet.winningRuns || 0
      }));

      // Group and accumulate runs and results
      console.log(allMetBets, "allBets all result")
      const sessionRuns = allMetBets.reduce((acc, bet) => {
        if (!acc[bet.label]) {
          acc[bet.label] = { 
            label: bet.label, 
        ...
        acc[bet.label].yesRuns = bet.yesRuns;
        acc[bet.label].noRuns = bet.noRuns;
        // Update result if it's not pending
        if (bet.result && bet.result !== 'Pending') {
          acc[bet.label].result = bet.result;
        }
        // Update winning runs if available
        if (bet.winningRuns) {
          acc[bet.label].winningRuns = bet.winningRuns;
        }
        return acc;
      }, {});
  ...
  }, [match]); // Added match to dependency array
