...
const Papu = () => {
  ...
  const handleBetChange = (bet, profit) => {
    setSelectedBetAmount(bet);
    const selectedBetData = betPlaceAmt.find((item) => item.bet === bet);
    // console.log(selectedBetData)
    if (selectedBetData) {
      setExpectedProfit(selectedBetData.profit);
      // setTotalProfit(selectedBetData.profit + (userData?.wallet?.balance || 0));
                ...
                {betPlaceAmt.map(({ bet, profit }) => (
                  <button
                    ...
                    style={{
                      ...
                      background: selectedBetAmount === bet ? '#2a3a4a' : '#141a24',
                      width: "21%",
                      color: "white",
                      border: selectedBetAmount === bet ? '2px solid #4CAF50' : '2px solid transparent',
                      cursor: 'pointer',
                      fontWeight: 'bold',
                      transition: 'all 0.3s ease',
                      transform: selectedBetAmount === bet ? 'scale(1.05)' : 'scale(1)',
                      boxShadow: selectedBetAmount === bet ? '0 0 10px rgba(76, 175, 80, 0.5)' : 'none'
                    }}
                  >
                    {bet}
                  </button>
