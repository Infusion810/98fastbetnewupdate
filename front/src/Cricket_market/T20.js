...
//       // Calculate amount to deduct based on bet type
//       let amountToDeduct = 0;
//       if (selectedBet.type === "Lgaai") {
//         amountToDeduct = stake;
//       } else if (selectedBet.type === "khaai") {
//         amountToDeduct = (stake * parseFloat(selectedBet.odds)) / 100;
//       }
...
//       let newTeam1Winnings = 0;
//       let newTeam2Winnings = 0;
...
//       // Update team winnings state
//       setTeam1Winnings(prev => prev + newTeam1Winnings);
//       setTeam2Winnings(prev => prev + newTeam2Winnings);

//       // Calculate new exposure
//       const newExposure = Math.abs(Math.min(team1Winnings + newTeam1Winnings, team2Winnings + newTeam2Winnings));
      
//       // Update balance and exposure
//       setBalance(prevBalance => prevBalance - amountToDeduct);
//       setMarketOddsExposure(newExposure);
//       setPrevMarketOddsExposure(newExposure);
...
//         stake: stakeValue,
//         teamAProfit: Number((team1Winnings + newTeam1Winnings).toFixed(2)),
//         teamBProfit: Number((team2Winnings + newTeam2Winnings).toFixed(2)),
//         balance: Number((balance - amountToDeduct).toFixed(2)),
//         exposure: marketOddsExposure + Number(newExposure.toFixed(2)),
//         marketType: 'matchOdds',
//         rate: selectedBet.odds,
//         userId: JSON.parse(localStorage.getItem('user'))?.id,
//         match: match,
...
//         // setSubmitClick((prev) => (prev + 1))
//         setBalanceChange(false);
//         /* Commented out the old bet creation logic
//         if (submitClick >= 1) {
//           const placeBet = async () => {
//             const now = new Date();
//             const newBet = {
    ...
    try {
      ...
      if (Math.abs(newExposure) > balance) {
        if (Math.abs(Math.min(newTeam1Winnings, newTeam2Winnings)) != (marketOddsExposure + Math.abs(newExposure))) {
          ...
          // Check if this is a Khaai bet after a Lgaai bet
          if (selectedBet.type === "khaai" && team1Winnings < 0) {
            // Calculate available balance including potential profit from Lgaai bet
            const availableBalance = balance + Math.abs(team1Winnings);
            if (Math.abs(newExposure) <= availableBalance) {
              // Allow the bet to proceed
              setPrevMarketOddsExposure(prevMarketOddsExposure + newExposure);
              setMarketOddsExposure(marketOddsExposure + Math.abs(newExposure));
              setBalance(balance - Math.abs(newExposure));
            } else {
