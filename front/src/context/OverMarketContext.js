...
//     try {
//       const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/cricket-market/${userId}/${localStorage.getItem("match")}`);
...
// // console.log( localStorage.getItem("match"), "match" )
...
//         odds: parsedOdds,
//         runs: parsedRuns,
//         matchName: localStorage.getItem("match"),
...
//       const betRecord = {
//         userId: JSON.parse(localStorage.getItem('user')).id,
//         matbet: bet.label || bet.matbet,
//         mode: bet.type.toLowerCase(),
//         odds: parsedOdds,
//         rate: parsedOdds,
//         stake: parsedStake,
//         profitA: bet.type.toLowerCase() === 'yes' ? (parsedStake * parsedOdds / 100) : -parsedStake,
//         profitB: bet.type.toLowerCase() === 'no' ? parsedStake : -(parsedStake * parsedOdds / 100),
//         balance: newBalance,
//         exposure: currentBetExposure,
//         yesRuns: bet.type.toLowerCase() === 'yes' ? parsedRuns : undefined,
//         noRuns: bet.type.toLowerCase() === 'no' ? parsedRuns : undefined,
//         matchName: localStorage.getItem("match")
//       };
//       //api call
//       console.log(betRecord, "betRecord")
...
export const OverMarketProvider = ({ children }) => {
  ...
  const handleOverBetPlacement = useCallback(async (bet, stake, balance, setBalance, setExposure) => {
    try {
      ...
      // Get existing bets for this run value
      const existingBets = sessionBets[parsedRuns] || [];
      
      // Check if user already has a bet of the opposite type for this run
      const hasOppositeBet = existingBets.some(existingBet => 
        existingBet.type.toLowerCase() !== bet.type.toLowerCase()
      );

      if (hasOppositeBet) {
        setInsufficientBalanceMessage('You already have a bet of the opposite type for this run value.');
        setInsufficientBalancePopup(true);
        return false;
      }
